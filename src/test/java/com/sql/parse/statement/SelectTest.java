...
//        select.addColumn(column1, "c1");
            ...
            switch (SqlParseConfig.getHandle()) {
                case "com.github.jsqlparser": {
                    Assert.assertEquals(
                            SqlFormatter.format(select.getSql()),
                            SqlFormatter.format("SELECT column1, column2 c2, column3 c3 FROM table1 LIMIT 500")
                    );
                    Assert.assertEquals(
                            SqlFormatter.format(select.getSql(false)),
                            SqlFormatter.format("SELECT column1, column2 c2, column3 c3 FROM table1")
                    );
                    break;
                }
                case "com.alibaba.druid": {
...
//            select.addColumn("t."+column1+" c1");
            ...
            Assert.assertEquals(
                    SqlFormatter.format(select.getSql()),
                    SqlFormatter.format("SELECT t.column1, t.column2, t.column3 FROM table1 t LIMIT 500")
            );
            Assert.assertEquals(
                    SqlFormatter.format(select.getSql(false)),
                    SqlFormatter.format("SELECT t.column1, t.column2, t.column3 FROM table1 t")
            );
        } catch (Exception e) {
            Assert.assertEquals(SqlParseConfig.getHandle(), "com.alibaba.druid");
        }
    }
